...
fn test_multiline_with_scanner() {
    ...
    let line_strs: Vec<String> = lines.into_iter().map(|e| e.message).collect();

    let pattern = Regex::new(r"^\d{4}-\d{2}-\d{2}").unwrap();
    let events = assemble_multiline(line_strs, Some(&pattern), 1000);
...
fn test_emf_single_line_passthrough() {
    ...
    let events = assemble_multiline(line_strs, None, 1000);
