...
use gg_log_manager::config::parse_config;
...
fn test_lld_j5_system_health_component() {
    ...
    assert_eq!(system_health.source.disk_space_limit_unit, "MB");
    assert!(system_health.source.delete_log_file_after_cloud_upload);
    assert_eq!(
        system_health.source.multi_line_start_pattern,
        Some("^\\{".to_string())
    );
    // uploadIntervalSec: 60 overrides global periodicUploadIntervalSec: 300
    assert_eq!(system_health.source.upload_interval_sec, Some(60));
    // log_group_name is None - should be auto-derived
...
fn test_lld_j5_upload_interval_override() {
    ...
    // Per-source uploadIntervalSec (60) overrides global periodicUploadIntervalSec (300)
    assert_eq!(system_health.source.upload_interval_sec, Some(60));
    assert_eq!(config.periodic_upload_interval_sec, 300);
}
...
fn test_lld_j5_docker_health_component() {
    ...
    // Custom log group name specified
    assert_eq!(
    ...
    assert_eq!(docker_health.source.disk_space_limit_unit, "MB");
    assert!(!docker_health.source.delete_log_file_after_cloud_upload);
    assert_eq!(docker_health.source.minimum_log_level, "DEBUG");
    // No uploadIntervalSec - should use global
...
fn test_lld_j5_device_bridge_component_defaults() {
    ...
    assert_eq!(device_bridge.source.disk_space_limit_unit, "MB");
    // Missing optional fields get defaults
    assert!(!device_bridge.source.delete_log_file_after_cloud_upload);
    assert_eq!(device_bridge.source.minimum_log_level, "INFO"); // default
    assert!(device_bridge.log_group_name.is_none()); // auto-derived
...
fn test_lld_j5_insights_system_config() {
    ...
    assert_eq!(insights.source.disk_space_limit_unit, "MB");
    assert!(insights.source.delete_log_file_after_cloud_upload);
    assert_eq!(insights.source.minimum_log_level, "WARN");
...
fn test_auto_derived_log_group_name() {
    ...
    // When log_group_name is None, it should be auto-derived
