...
/// Compute SHA-256 hash of the first line (up to newline) or first 1024 bytes of a file.
/// Returns hex-encoded hash string.
///
/// This matches the Java LogManager's `LogFile` identity mechanism:
/// - Read up to 1024 bytes
/// - If there's a newline within those bytes, hash only up to (and including) the first newline
/// - If no newline, hash all bytes read
pub fn compute_content_hash(path: &Path) -> std::io::Result<String> {
    let mut file = File::open(path)?;
    let mut buffer = [0u8; 1024];
    let bytes_read = file.read(&mut buffer)?;
    let data = &buffer[..bytes_read];
    // Match Java behavior: hash first line (up to newline) if present, otherwise hash all bytes read
    let hash_input = match data.iter().position(|&b| b == b'\n') {
        Some(pos) => &data[..=pos], // include the newline
        None => data,
    };
    ...
    loop {
        ...
        match reader.read_line(&mut line) {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::InvalidData => {
                tracing::warn!(path = %path.display(), offset = current_offset, "Invalid UTF-8, skipping byte");
                current_offset += 1;
                reader.get_mut().seek(SeekFrom::Start(current_offset))?;
                continue;
            }
            Err(_) => {
                return Ok((events, current_offset));
            }
        }
        ...
        let timestamp = extract_timestamp(trimmed).unwrap_or(default_timestamp);
        let message = truncate_message(trimmed);
        events.push(LogEvent { timestamp, message });
    }

    tracing::debug!(path = %path.display(), event_count = events.len(), new_offset = current_offset, "File read complete");
...
mod tests {
    ...
    fn test_read_file_with_invalid_utf8() {
        let mut file = NamedTempFile::new().unwrap();
        // Write valid UTF-8, then invalid byte, then more valid UTF-8
